...
class MemTracker(object):
    ...
    def write_file(self, tensor_shape):
        with open(self.gpu_profile_fn, "a") as f:
            tensor_shape = str(list(tensor_shape))
            tensor_used_memo = f"{self.get_tensor_usage():<7.1f}"
            allocated_memo = f"{self.get_allocate_usage():<7.1f}"
            string = f"{tensor_shape} {tensor_used_memo} {allocated_memo}\n"
            f.write(string)
        return
